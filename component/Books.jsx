...
export default function BooksPremium() {
  ...
  useEffect(() => {
    ...
    cards.forEach((card, index) => {
      gsap.fromTo(
        card,
        {
          y: isMobile ? 100 : 220, // 👈 from bottom (responsive)
          opacity: 0,
          scale: 0.95,
        },
        {
          ...
          scale: 1,
          ease: "power4.out",
          delay: index * 0.08, // 👈 subtle stagger feel
          scrollTrigger: {
            trigger: card,
            start: "top 90%",
            end: "top 40%",
  ...
  return (
    <section
      id="Books"
      ref={containerRef}
      className="relative bg-mist-600"
    >
      {/* Heading */}
      <div className="flex justify-center items-center py-10 mb-6">
        <h2 className="text-3xl md:text-5xl font-light tracking-[0.3em] text-gray-800 text-center">
          BIBLIOGRAPHY
          <span className="block w-16 h-px bg-gray-400 mx-auto mt-4"></span>
        </h2>
      </div>
      ...
      {books.map((book, index) => (
        ...
        >
          <div
            className="book-card w-full max-w-6xl
            bg-white/10 backdrop-blur-xl border border-white/20
            rounded-2xl shadow-2xl p-6 md:p-12"
          >
